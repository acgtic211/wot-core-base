...
export class HTTPManager {
	constructor(config) {
		...
		this.client = new HTTPClient_v1_0_0();
	}
	...
	initHTTPManager(mqttGatewayInfo, request) {
...
// console.log('close')								
	...
	terminateHTTPManager(hostname, port, thingId, topic) {
		return new Promise((resolve, reject) => {
	...
	processSubscribe(mqttGatewayInfo, request) {
		return new Promise((resolve, reject) => {
			...
			this.initHTTPManager(mqttGatewayInfo, request)
			.then(() => {
...
// console.log('******** terminateHTTPManager', hostname, port, thingId, this.hosts[hostname][port][thingId].topic)										
										this.terminateHTTPManager(hostname, port, thingId, this.hosts[hostname][port][thingId].topic)
										.then(() => {})
										.catch((error) => {})
										.finally(() => {
	...
	consumeService(request) {
		return new Promise((resolve, reject) => {
			...
			switch(request.op) {
				...
				case 'unsubscribeevent':
				default:
					reject('ConsumeService HTTP error...');
					break;
			}		
		});
	}
