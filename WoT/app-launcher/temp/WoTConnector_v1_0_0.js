...
export class WoTConnector_v1_0_0 {
	...
	link(name, inputs, options) {
		return new Promise((resolve1, reject1) => {
    		...
    		.then((sourceThingDescription) => {
                let targets = [];
                
    			// this.count += 1;

    			// let sourceId = 'Servient_Example_1_CT__CT__Connector_1' + '__' + 'CT' + this.count + '__' + sourceThingDescription.id;
    			...
    			if(sourceEventName === null) {
        			...
        			.then((values) => {
    					...
    					sourceThing.subscribeEvent(sourceEventName, 
    						(eventData) => {
// if(sourceEventName === 'onScheduled2') console.log('--------------- 1', eventData)			
...
// console.log(value[c[0].key[0]][c[0].key[1]], c[0].value, p)        								        
			...
			.catch((error) => {
				reject('No sourceThingDescription...');
			});
		});
	}
