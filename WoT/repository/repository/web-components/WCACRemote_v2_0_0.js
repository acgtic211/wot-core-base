...
export class WCACRemote_2 extends WCThingPack1_v1_0_0 {
    constructor() {
        super();
    ...
    async connectedCallback() {
	    ...
	    let status = this.getItem(this.onChangeStatusHref);
    ...
    initACRemote(component) {
		...
		let s = component.getItem(component.getAttribute('status-href'));
		...
		component.httpsClient.fetch(s.hostname, s.port, s.pathname, 'get', 'application/json', null)
		.then((status) => {
			console.log(status);
...
customElements.define('wc-acremote-2', WCACRemote_2);