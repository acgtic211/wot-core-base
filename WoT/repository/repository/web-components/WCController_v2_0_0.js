...
export class WCController_v2_0_0 extends WCThingPack1_v1_0_0 {
	constructor() {
		super();
	...
	connectedCallback() {
		...
		this.CL2ToggleHref = this.getAttribute('colorlight-2-toggle-href');
		this.CL2StateHref = this.getAttribute('colorlight-2-state-href');
		this.CL2ChangeStateHref = this.getAttribute('colorlight-2-changestate-href');

		this.CL3ToggleHref = this.getAttribute('colorlight-3-toggle-href');
        this.CL3StateHref = this.getAttribute('colorlight-3-state-href');
		this.CL3ChangeStateHref = this.getAttribute('colorlight-3-changestate-href');
...
//					div3b.appendChild(pstate);		
	...
	getState(component) {
		let item = component.getItem(component.getAttribute('colorlight-2-state-href'));
	...
	changeState(component){
		...
		let item = component.getItem(component.getAttribute('colorlight-2-state-href'));
        let item2 = component.getItem(component.getAttribute('colorlight-3-state-href'));
		...
		component.httpClient.fetch(item.hostname, item.port, item.pathname, 'get', 'application/json', null)
        .then((props) => {
            let cs = component.getItem(component.getAttribute('colorlight-2-changestate-href'));
        ...
        .catch((error) => {
            console.log("Error changing state: ColorLight 2");
        });

        component.httpClient.fetch(item2.hostname, item2.port, item2.pathname, 'get', 'application/json', null)
        .then((props) => {
            let cs = component.getItem(component.getAttribute('colorlight-3-changestate-href'));
            component.httpClient.fetch(cs.hostname, cs.port, cs.pathname, 'put', 'application/json', newState);
			this.getState(component, props.state);
        })
        .catch((error) => {
            console.log("Error changing state: ColorLight 3");
        });
	}
	...
	toggle(component){
        ...
        component.httpClient.fetch(item.hostname, item.port, item.pathname, 'get', 'application/json', null)
        .then((props) => {
            let toggle = component.getItem(component.getAttribute('colorlight-2-toggle-href'));
        ...
        component.httpClient.fetch(item2.hostname, item2.port, item2.pathname, 'get', 'application/json', null)
        .then((props) => {
            let toggle = component.getItem(component.getAttribute('colorlight-3-toggle-href'));
            component.httpClient.fetch(toggle.hostname, toggle.port, toggle.pathname, 'put', 'application/json', {"on":!props.state.on});
			this.getState(component, props.state);
        })
        .catch((error) => {
...
customElements.define('wc-controller2', WCController_v2_0_0);
