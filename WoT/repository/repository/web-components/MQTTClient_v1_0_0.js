...
export class MQTTClient_v1_0_0 {
	...
	connect(hostname, port, resubscribe, externalServer) {
		let options = {
			...
			clientId: "mqttClientWC" + Math.random().toString(16).substr(2, 8)
		};
		...
		if(this.client === null) {
            ...
            /*if((this.externalServer === undefined) || (this.externalServer === false)) {
    			this.client = MQTT.connect('ws://' + hostname + ':' + port, options);
		    } else {
    			this.client = MQTT.connect('mqtt://' + hostname + ':' + port, options);
		    }*/
		}
	}
